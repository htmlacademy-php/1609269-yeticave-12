<main>
    <form class="form form--add-lot container <?=($status['form']) ? "":"form--invalid"?>" action="/add.php" method="post" enctype="multipart/form-data"> <!-- form--invalid -->
      ...
      <div class="form__container-two">
        <div class="form__item <?=($status['lot-name']) ? "" : "form__item--invalid"?>"> <!-- form__item--invalid -->
          <label for="lot-name">Наименование <sup>*</sup></label>
          <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value=<?=getPostVal("lot-name")?>>
          <span class="form__error" ><?=($status['lot-name']) ? "" : $errors['lot-name']?></span>
        </div>
      ...
      <div class="form__item form__item--file <?=($status['lot-img']) ? "" : "form__item--invalid"?>"> <!-- Попытался добавить ошибку здесь -->
        ...
        <div class="form__input-file">
          ...
          <span class="form__error"><?=($status['lot-img']) ? "" : $errors['lot-img']?></span>
        </div>
      </div>
      <div class="form__container-three">
        <div class="form__item form__item--small <?=($status['lot-rate']) ? "" : "form__item--invalid" ?>">
          <label for="lot-rate">Начальная цена <sup>*</sup></label>
          <input id="lot-rate" type="text" name="lot-rate" placeholder="0"  value=<?=getPostVal("lot-rate")?>>
          <span class="form__error"><?=($status['lot-rate']) ? "" : $errors['lot-rate']?></span>
        </div>
        <div class="form__item form__item--small <?=($status['lot-step']) ? "" : "form__item--invalid" ?>">
          <label for="lot-step">Шаг ставки <sup>*</sup></label>
          <input id="lot-step" type="text" name="lot-step" placeholder="0" value=<?=getPostVal("lot-step")?>>
          <span class="form__error"><?=($status['lot-step']) ? "" : $errors['lot-step']?></span>
        </div>
        <div class="form__item <?=($status['lot-date']) ? "" : "form__item--invalid" ?>">
          ...
          <span class="form__error"><?=($status['lot-date']) ? "" : $errors['lot-date']?></span>
        </div>
      </div>
