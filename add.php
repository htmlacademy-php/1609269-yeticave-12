...
if( !empty($_POST['lot-name']) || !empty($_POST['category']) || !empty($_POST['message']) ||
    !empty($_POST['lot-rate']) || !empty($_POST['lot-step']) || !empty($_POST['lot-date']) ||
    !empty($_FILES['lot-img'])){
        if( $status['name'] || $status['category'] || $status['message'] ||
            $status['rate'] || $status['step'] || $status['date'] ||
            $status['file']){
                $status['form'] = false;
                if(empty($_POST['lot-name'])){
                    $status['name'] = false;
